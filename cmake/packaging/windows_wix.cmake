...
# If wix.exe already exists in the tool path, skip installation
if(NOT EXISTS "${WIX_TOOL_PATH}/wix.exe")
    # Try to find wix globally (e.g. installed via dotnet tool install --global wix)
    find_program(WIX_GLOBAL_EXECUTABLE wix)
    if(WIX_GLOBAL_EXECUTABLE)
        message(STATUS "Found globally installed WiX: ${WIX_GLOBAL_EXECUTABLE}, creating stub")
        # Create a forwarding script so the rest of the cmake can use WIX_TOOL_PATH/wix
        file(WRITE "${WIX_TOOL_PATH}/wix.cmd" "@\"${WIX_GLOBAL_EXECUTABLE}\" %*\r\n")
        set(WIX_EXECUTABLE "${WIX_GLOBAL_EXECUTABLE}")
    else()
        ...
        if(NOT WIX_INSTALL_RESULT EQUAL 0)
            message(WARNING "Failed to install WiX tools locally, WiX packaging will be skipped.\n"
                    "     Error: ${WIX_INSTALL_OUTPUT}")
            return()
        endif()
    endif()
else()
    message(STATUS "WiX already installed at ${WIX_TOOL_PATH}/wix.exe, skipping download.")
endif()
...
if(NOT WIX_UI_INSTALL_RESULT EQUAL 0)
    message(WARNING "Failed to install WiX UI extension, WiX packaging may not work correctly.\n"
            "     Error: ${WIX_UI_INSTALL_OUTPUT}")
endif()
...
if(NOT WIX_UTIL_INSTALL_RESULT EQUAL 0)
    message(WARNING "Failed to install WiX Util extension, WiX packaging may not work correctly.\n"
            "     Error: ${WIX_UTIL_INSTALL_OUTPUT}")
endif()
