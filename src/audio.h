...
namespace audio {
  ...
  /**
   * @brief 获取音频上下文的引用。
   * @returns 指向音频上下文的共享指针引用。
   * @note 除了用于配置之外，它还可用于延长音频接收器的生命周期，以便在早期捕获接收器并稍后恢复它。
   * @examples
  ...
  /**
   * @brief 检查音频上下文所关联的音频接收端是否可用。
   * @returns 如果可用（并且可能可以恢复），则返回true，否则返回false。
   * @note 有助于延迟释放音频上下文共享指针（该指针尝试恢复原始接收器）。
   *
   * @examples
   * audio_ctx_ref_t audio = get_audio_ctx_ref()
   * if (audio.get()) {
